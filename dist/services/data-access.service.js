...
let DataAccessService = class DataAccessService {
    constructor(databaseService, mockDataService) {
    ...
    async transaction(callback) {
        if (this.isDatabaseAvailable) {
            ...
            const mockClient = {
                query: async (text, params) => this.getMockQueryResult(text, params),
                release: () => { }
            };
            return await callback(mockClient);
        }
    }
    getMockQueryResult(text, params) {
    ...
    async createOrganization(organization) {
        if (this.isDatabaseAvailable) {
            const result = await this.databaseService.query(`INSERT INTO organizations (name, description, status, metadata, created_at, updated_at)
         VALUES ($1, $2, $3, $4, NOW(), NOW())
         RETURNING *`, [organization.name, organization.description, organization.status, organization.metadata]);
    ...
    async getOrganization(organizationId) {
        ...
        if (this.isDatabaseAvailable) {
            ...
            const result = await this.databaseService.query('SELECT * FROM organizations WHERE id = $1', [organizationId]);
    ...
    async updateOrganization(organizationId, updates) {
        if (this.isDatabaseAvailable) {
            ...
            const result = await this.databaseService.query(`UPDATE organizations 
    ...
    async listOrganizations(limit, offset) {
        ...
        if (this.isDatabaseAvailable) {
            ...
            const result = await this.databaseService.query('SELECT * FROM organizations ORDER BY created_at DESC LIMIT $1 OFFSET $2', [limit, offset]);
            return {
    ...
    async setupOrganization(setupData) {
        if (this.isDatabaseAvailable) {
           ...
           RETURNING *`, [
                ...
                let rootOU = null;
                if (setupData.rootOU) {
                    const ouResult = await client.query(`INSERT INTO organization_units 
           ...
           RETURNING *`, [
                ...
                const roleResult = await client.query(`INSERT INTO roles 
...
exports.DataAccessService = DataAccessService = __decorate([
    ...
    __param(1, (0, inversify_1.inject)('MockDataService')),
